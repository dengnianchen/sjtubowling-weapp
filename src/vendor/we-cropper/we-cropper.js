...
(function(global, factory) {
	...
	function createCommonjsModule(fn, module) {
		return module = { exports: {} }, fn(module,
		...
		(function(root) {
			...
			var decode = function(input) {
				...
				while (++position < length) {
					...
					bitStorage = bitCounter % 4
						? bitStorage * 64 + buffer
						: buffer;
	...
	function methods() {
		...
		self.updateCanvas = function() {
			if (self.croperTarget) {
				//  画布绘制图片
				self.ctx.drawImage(self.croperTarget, self.imgLeft, self.imgTop,
					self.scaleWidth, self.scaleHeight);
			}
		...
		self.pushOrign = function(src) {
			...
			isFunction(self.onBeforeImageLoad) &&
			...
			wx.getImageInfo({
				...
				success: function success(res) {
					...
					if (innerAspectRadio < width / height) {
						...
						self.rectY = y -
							Math.abs((height - self.baseHeight) / 2);
					} else {
						self.rectY = y;
		...
		self.getCropperRegion = function() {
			var args = [], len = arguments.length;
			while (len--) args[len] = arguments[len];
			...
			var cropX = (x - self.imgLeft) / self.scaleWidth;
			var cropY = (y - self.imgTop) / self.scaleHeight;
			var cropWidth = width / self.scaleWidth;
			var cropHeight = width / self.scaleHeight;
			
			isFunction(fn) && fn.call(self, {
				x: cropX,
				y: cropY,
				width: cropWidth,
				height: cropHeight
	...
	function update() {
		...
		self.__twoTouchMove = function(touch0, touch1) {
			...
			self.newScale = getNewScale(oldScale, oldDistance, zoom, touch0,
				touch1);
	...
	function cut() {
		...
		self.setBoundStyle = function(ref) {
			...
			var mask = ref.mask;
			if (mask === void 0) mask = 'rgba(0, 0, 0, 0.5)';
			...
			self.ctx.fillRect(x + width, 0, boundWidth - x - width,
				boundHeight);
			self.ctx.fill();
